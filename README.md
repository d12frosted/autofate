...
- **Unreachable fates are dropped.** If a trip stops getting closer (20s without closing in by 10y, pathfind waits not counted), we try another landing spot in the fate, and after three we skip that fate for 10 minutes. This is what happens when vnavmesh can't route to a fate at all, e.g. one on an Elpis island high above us.
